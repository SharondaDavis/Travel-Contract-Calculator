    ...
    if (fuelPrices && !assignment.fuelCostPerGallon && assignment.transportationType === 'personal') {
      const syntheticEvent = {
        target: {
          ...
          value: fuelPrices.regular.toString()
        }
      } as React.ChangeEvent<HTMLInputElement>;
      
      handleInputChange(syntheticEvent, assignment.id);
    }
          ...
          <>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {fuelPrices && <span className="ml-2 text-xs text-green-500">Current local price: ${fuelPrices.regular.toFixed(2)}</span>}
              </label>
              <input
                type="number"
                name="fuelCostPerGallon"
